...
m.edges_1 = pyo.Constraint(m.Nt, m.V, m.T, rule=edges_1_rule)
...
m.simmetry_breaking = pyo.ConstraintList()    
def simmetry_breaking_rule(m,k,t):
    for v in range(len(data['V'])-1):
        if (data['V'][v], k) in m.V_K and (data['V'][v+1], k) in m.V_K:
            m.simmetry_breaking.add(m.z['Node0',data['V'][v+1],t]<=m.z['Node0',data['V'][v],t])
        
for k in m.K:
    for t in m.T:
        simmetry_breaking_rule(m,k,t)
